package com.sj.sound_diary.service.impl;

import java.util.ArrayList;
import java.util.List;
import java.util.Map;
import java.util.Random;

import org.springframework.http.HttpEntity;
import org.springframework.http.HttpHeaders;
import org.springframework.stereotype.Service;
import org.springframework.web.client.RestTemplate;
import org.springframework.web.util.UriComponentsBuilder;

import com.sj.sound_diary.dto.TrackDto;
import com.sj.sound_diary.service.SpotifyService;
import com.sj.sound_diary.util.JsonUtils;
import com.sj.sound_diary.util.RestTemplateUtils;

import lombok.RequiredArgsConstructor;

@Service
@RequiredArgsConstructor
public class SpotifyServiceImpl implements SpotifyService {

    private final RestTemplate restTemplate;

    @Override
    public List<TrackDto> searchTracks(String query, String accessToken) {
        String url = UriComponentsBuilder.fromUriString("https://api.spotify.com/v1/search")
                .queryParam("q", query)
                .queryParam("type", "track")
                .queryParam("limit", "10")
                .build()
                .toUriString();

        return executeTrackSearch(url, accessToken);
    }
    
    @Override
    public TrackDto getRecommendation(String artistId, String excludeTrackUri, String accessToken) {
    	HttpEntity<Void> entity = buildAuthEntity(accessToken);

        // 1단계: 이 아티스트의 앨범/싱글 목록 조회 (최대 10개)
        String albumUrl = "https://api.spotify.com/v1/artists/" + artistId
                + "/albums?include_groups=album,single&limit=10";

        Map<String, Object> albumResponse = RestTemplateUtils.get(restTemplate, albumUrl, entity, Map.class);
        List<Map<String, Object>> albums = JsonUtils.field(albumResponse, "items");

        if (albums == null || albums.isEmpty()) {
            return null;
        }

        // 2단계: 앨범 중 하나를 무작위로 선택
        Map<String, Object> randomAlbum = albums.get(new Random().nextInt(albums.size()));
        String albumId = JsonUtils.field(randomAlbum, "id");

        List<Map<String, Object>> albumImages = JsonUtils.field(randomAlbum, "images");
        String albumImageUrl = (albumImages == null || albumImages.isEmpty())
                ? null : JsonUtils.field(albumImages.get(0), "url");

        // 3단계: 그 앨범의 트랙 목록 조회
        String tracksUrl = "https://api.spotify.com/v1/albums/" + albumId + "/tracks?limit=10";
        Map<String, Object> trackResponse = RestTemplateUtils.get(restTemplate, tracksUrl, entity, Map.class);
        List<Map<String, Object>> items = JsonUtils.field(trackResponse, "items");

        if (items == null || items.isEmpty()) {
            return null;
        }

        List<TrackDto> candidates = new ArrayList<>();
        for (Map<String, Object> item : items) {
            List<Map<String, Object>> artists = JsonUtils.field(item, "artists");

            candidates.add(TrackDto.builder()
                    .name(JsonUtils.field(item, "name"))
                    .artistName(JsonUtils.field(artists.get(0), "name"))
                    .artistId(JsonUtils.field(artists.get(0), "id"))
                    .albumImageUrl(albumImageUrl)   // 앨범 트랙 API엔 이미지가 없어서 앨범 정보에서 가져온 값 사용
                    .uri(JsonUtils.field(item, "uri"))
                    .build());
        }

        // 방금 재생한 곡 제외
        candidates.removeIf(t -> t.getUri().equals(excludeTrackUri));

        if (candidates.isEmpty()) {
            return null;
        }

        return candidates.get(new Random().nextInt(candidates.size()));
    }
    
    // 검색 API 호출 + TrackDto 변환 (공통 로직)
    private List<TrackDto> executeTrackSearch(String url, String accessToken) {
    	HttpEntity<Void> entity = buildAuthEntity(accessToken);

        Map<String, Object> response = RestTemplateUtils.get(restTemplate, url, entity, Map.class);
        Map<String, Object> tracks = JsonUtils.field(response, "tracks");
        List<Map<String, Object>> items = JsonUtils.field(tracks, "items");

        List<TrackDto> result = new ArrayList<>();
        for (Map<String, Object> item : items) {
            List<Map<String, Object>> artists = JsonUtils.field(item, "artists");
            Map<String, Object> album = JsonUtils.field(item, "album");
            List<Map<String, Object>> images = JsonUtils.field(album, "images");

            result.add(TrackDto.builder()
                    .name(JsonUtils.field(item, "name"))
                    .artistName(JsonUtils.field(artists.get(0), "name"))
                    .artistId(JsonUtils.field(artists.get(0), "id"))
                    .albumImageUrl(images.isEmpty() ? null : JsonUtils.field(images.get(0), "url"))
                    .uri(JsonUtils.field(item, "uri"))
                    .build());
        }
        return result;
    }
    //공통 로직 분리
    private HttpEntity<Void> buildAuthEntity(String accessToken) {
        HttpHeaders headers = new HttpHeaders();
        headers.set("Authorization", "Bearer " + accessToken);
        return new HttpEntity<>(headers);
    }
}